...
def main():
    ...
    # 1. TẠO ẢNH NHÂN VẬT (CHỈ 1 LẦN)
    char_img = get_path("assets", "temp", f"{episode_id}_raw_ai.png")
    if not os.path.exists(char_img):
        char_img = generate_character_image(data.get('Name'), char_img)

    # 2. XỬ LÝ VIDEO DÀI
    long_video_id = None
    long_res = generate_long_script(data)
    
    # SỬA LỖI TẠI ĐÂY: Kiểm tra long_res là dict trước khi truy cập
    if long_res and isinstance(long_res, dict):
        ...
        if tts:
            video = create_video(tts, episode_id, custom_image_path=char_img, title_text=meta["youtube_title"])
            if video:
                thumb = get_path("outputs", "thumbnails", f"{episode_id}_thumb.jpg")
                add_text_to_thumbnail(char_img, meta["youtube_title"], thumb)
                
                sched_long = (now + timedelta(hours=2)).isoformat()
                up_res = upload_video(video, meta, thumb, scheduled_time=sched_long)
                if isinstance(up_res, dict): long_video_id = up_res.get('video_id')
    ...
    if os.path.exists(long_txt):
        ...
        for i, t in enumerate(short_tasks):
            ...
            if s_tts:
                ...
                if s_video:
                    sched_s = (now + timedelta(hours=2 + (i*4))).isoformat()
                    s_meta = {"Title": f"{s_title} #Shorts", "Summary": f"Xem bản đầy đủ: {related_url}"}
                    upload_video(s_video, s_meta, scheduled_time=sched_s)

    cleanup_temp_files(episode_id, data.get('text_hash'))
    logger.info("✅ HỆ THỐNG ĐÃ PHỤC HỒI VÀ HOÀN TẤT!")
